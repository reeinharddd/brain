...
brain_dir = pathlib.Path(
    os.environ.get("BRAIN_DIR", str(pathlib.Path.home() / ".brain"))
)
...
opencode = {
    ...
    "mcp": {
        "memory": {
            ...
            "args": [
                "-lc",
                'npx -y @modelcontextprotocol/server-memory "$HOME/.brain/memory"',
            ],
        },
        "filesystem": {
            "type": "local",
        ...
        "context7": {
            "type": "local",
            "command": "bash",
            "args": ["-lc", "npx -y @upstash/context7-mcp@latest"],
        },
        "github": {
            "type": "local",
            "command": "bash",
        ...
        "brain-rules": {
            "type": "local",
            "command": "python3",
            "args": [str(brain_dir / "mcp" / "brain-mcp-server" / "server.py")],
        },
        "skill-ninja": {
            "type": "local",
            "command": "docker",
            "args": ["run", "-i", "--rm", "aktsmm/skill-ninja-mcp-server"],
        },
        "duckduckgo": {
            "type": "local",
            "command": "docker",
...
out_path.write_text(
    json.dumps(opencode, indent=2, ensure_ascii=False), encoding="utf-8"
)
