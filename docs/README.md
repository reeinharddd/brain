---
type: documentation-index
id: docs-main-index
title: Brain Repository Documentation
version: 2.2.0
status: active
date_created: 2026-04-07
language: en
category: documentation
---

## Documentation Home

Brain documentation is organized so architecture is closed before large implementation work proceeds.

Start here for the official direction of the project:

- [INDEX.md](/home/reeinharrrd/Work/Personal/brain/docs/INDEX.md)
- [brain-v2-target-architecture.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/brain-v2-target-architecture.md)
- [ADR Index](/home/reeinharrrd/Work/Personal/brain/docs/adr/README.md)
- [Architecture Index](/home/reeinharrrd/Work/Personal/brain/docs/architecture/README.md)

## Starting Points

- Just arrived: read [QUICK-START.md](/home/reeinharrrd/Work/Personal/brain/docs/QUICK-START.md)
- Need the target state: read [brain-v2-target-architecture.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/brain-v2-target-architecture.md)
- Need the decision trail: read [ADR Index](/home/reeinharrrd/Work/Personal/brain/docs/adr/README.md)
- Need the docs layout: read [STRUCTURE.md](/home/reeinharrrd/Work/Personal/brain/docs/STRUCTURE.md)
- Need testing guidance: read [Testing README](/home/reeinharrrd/Work/Personal/brain/docs/testing/README.md)

## Core Domains

- `adr/` for architecture decisions and rationale
- `architecture/` for design documents and target contracts
- `skills/` for skill-specific rules inside the artifact model
- `testing/` for the future-facing quality baseline and validation guidance
- `templates/` for approved templates

## Standards Reference

- [DOCUMENTATION-UNIFIED-SCHEMA.md](/home/reeinharrrd/Work/Personal/brain/docs/DOCUMENTATION-UNIFIED-SCHEMA.md)
- [documentation-enforcement-system.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/documentation-enforcement-system.md)
- [development-methodology.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/development-methodology.md)

## Key Architecture Documents

- [brain-v2-target-architecture.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/brain-v2-target-architecture.md)
- [repository-structure-v2.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/repository-structure-v2.md)
- [artifact-system-contract.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/artifact-system-contract.md)
- [identity-policy-security.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/identity-policy-security.md)
- [ai-runtime-and-context-optimization.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/ai-runtime-and-context-optimization.md)
- [memory-and-knowledge-architecture.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/memory-and-knowledge-architecture.md)
- [model-compatibility-and-capability-layering.md](/home/reeinharrrd/Work/Personal/brain/docs/architecture/model-compatibility-and-capability-layering.md)
- [testing-strategy.md](/home/reeinharrrd/Work/Personal/brain/docs/testing/testing-strategy.md)
- [Testing README](/home/reeinharrrd/Work/Personal/brain/docs/testing/README.md)

## Quality Principles

1. Factual, not aspirational by accident
2. Current, not historical unless explicitly archived
3. Linked, so decisions and designs are traceable
4. Canonical, so active ADRs override stray notes

## Contribution Rules

1. Put decisions in `docs/adr/`.
2. Put system contracts in `docs/architecture/`.
3. Update [INDEX.md](/home/reeinharrrd/Work/Personal/brain/docs/INDEX.md) when adding a new canonical entry point.
4. Keep documentation in English and aligned with the unified schema.
5. Treat domain `README.md` files as navigation hubs, not strict design docs.
