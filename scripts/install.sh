...
print_summary() {
  section "Result"
  if [ $ERRORS -eq 0 ]; then
    echo -e "\n  ${GREEN}${BOLD}[ok] Brain repo installed on $OS${RESET}"
    echo -e "  ${GREEN}Unified Architecture v2.0${RESET}"
  else
    echo -e "\n  ${YELLOW}${BOLD}[warn] Brain repo installed with $ERRORS issue(s)${RESET}"
    echo "  Run brain doctor for details"
  fi

  echo ""
  echo "  Active adapters:"
  [ -L "$HOME/.claude/CLAUDE.md" ]   && echo "    [ok] Claude Code"
  [ -L "$HOME/.cursorrules" ]         && echo "    [ok] Cursor"
  [ -L "$HOME/.windsurfrules" ]       && echo "    [ok] Windsurf"
  [ -L "$HOME/.gemini/GEMINI.md" ]    && echo "    [ok] Gemini CLI"
  [ -L "$HOME/.aider.conf.yml" ]      && echo "    [ok] Aider"

  echo ""
  echo -e "  ${BOLD}Quick Start (New Unified Architecture):${RESET}"
  echo "    1. Configure:        cp ~/.brain/brain.env.example ~/.brain/brain.env"
  echo "    2. Start ecosystem:  ~/.brain/bin/brain up"
  echo "    3. Check status:     ~/.brain/bin/brain status"
  echo "    4. Sync IDEs:        ~/.brain/bin/brain sync"
  echo ""
  echo "  Commands available:"
  echo "    brain up/down        - Start/stop ecosystem"
  echo "    brain status         - Show all services"
  echo "    brain doctor         - Health checks"
  echo "    brain sync           - Sync MCPs to IDEs"
  echo "    brain mcp list       - List MCPs"
  echo "    brain llm list       - List LLM models"
  echo ""
  echo "  Documentation: ~/.brain/ARCHITECTURE.md"
  echo ""
}
